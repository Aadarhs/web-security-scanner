...
async function scanOpenRedirect(targetUrl, httpClient) {
  ...
  try {
    ...
    // Test each common redirect parameter with each payload (parallel per param)
    for (const param of COMMON_REDIRECT_PARAMS) {
      const paramResults = await Promise.allSettled(REDIRECT_TEST_PAYLOADS.map(payload => {
        ...
        return httpClient.get(testUrl, {
          ...
          if (status >= 301 && status <= 308 && location && location.includes('example.com')) {
            return { type: 'server', param, payload, testUrl, status, location };
          }
          ...
          if (status === 200) {
            ...
            if ((hasJsRedirect || hasMetaRedirect) && hasReflectedPayload) {
              return { type: 'client', param, payload, testUrl, responseBody, hasJsRedirect, hasMetaRedirect, hasReflectedPayload };
            }
          }
          return null;
        });
        ...
        if (result.status === 'fulfilled' && result.value) {
          ...
          if (found.type === 'server') {
            vulnerabilities.push({
              ...
              description: `Parameter "${found.param}" redirects to user-controlled URL without validation. An attacker can use this to phish users.`,
              endpoint: baseUrl,
              parameter: found.param,
              payload: `${found.param}=${found.payload}`,
            ...
            vulnerabilities.push({
              ...
              description: `Parameter "${found.param}" is reflected in a redirect mechanism (JS or meta refresh). May allow phishing if controllable.`,
              endpoint: baseUrl,
              parameter: found.param,
              payload: `${found.param}=${found.payload}`,
              evidence: `Test URL: ${found.testUrl}\nJS Redirect: ${found.hasJsRedirect}\nMeta Refresh: ${found.hasMetaRedirect}\nPayload Reflected: ${found.hasReflectedPayload}\nClient-side redirects can be used for phishing.`,
